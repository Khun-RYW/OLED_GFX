...
uint8_t initCmdSSD1306[] = {// initialization for an SSD1306 controller
	...
	0x80,0xd5,0x80,0x80,	// setDisplayClockDiv	
	0x80,0xa8,0x80,0x37,	// setMultiplex, 63 lines
...
void OLED_GFX::setCursor(int row, int col, int lines){
	int hPos = (lines == 1 ? 6 : 10) * (col % (lines == 1 ? 21 : 13));
	bufPtr = bufr;
...
void OLED_GFX::printLarge(int row, int col, char *str){
	...
	if(row > 0)		mRow = (row - 1) % 7;		// keep in the range 0-6 no matter what the user sends
...
void OLED_GFX::printGFX(int col, int row, char *message){
	...
	while(*p  && bufferLen+32 < 128)	// stop if terminal null character or bitmap buffer near capacity
		if(*p == 0xe0 && (p[1] == 0xb8 || p[1] == 0xb9)){	// is it a Thai character?
			fnt = &thFont2;	// switch to Thai characters	// switch to Thai GFX struct
			if(p[1] == 0xb9)	bufferChar(p[2] - 0x41);
			if(p[1] == 0xb8)	bufferChar(p[2] - 0x81);
			p += 3;
		}else{
			fnt = &enFont2;	// switch to ASCII characters
			bufferChar(*p++);
		}
